...
ready(() => {
  ...
  if (is_reduced_motion) {
    console.info(
      "%cBased on the accessibility settings of your operating system, this website has limited animations.",
      "font-weight: bold"
    );
  }
  ...
  if (document.querySelector(".lemmony-read-time-counter")) {
    ...
    if (document.querySelector(".entry-content.wp-block-post-content")) {
      var post_content = document.querySelector(
    ...
    document
      .querySelector(".lemmony-read-time-counter")
      .prepend(read_time + " ");
  }
  ...
  if (typeof simpleParallax !== "undefined" && !is_reduced_motion) {
    var parallax = document.querySelectorAll(
      ".lemmony-parallax .wp-block-cover__image-background"
    );
    if (parallax) {
  ...
  var counters = document.querySelectorAll(
    ".wp-block-lemmony-companion-counter .lemmony-counter-wrapper"
  );
  if (counters) {
    counters.forEach((counter) => {
      new CountUp(
        counter.querySelector(".lemmony-counter-number"),
        counter.getAttribute("data-end"),
        {
          startVal: parseInt(counter.getAttribute("data-start")),
          enableScrollSpy: true,
          duration: !is_reduced_motion
            ? parseInt(counter.getAttribute("data-duration"))
            : 0,
          scrollSpyOnce: false,
          scrollSpyDelay: 100,
        }
    ...
    var callback2 = (entries) => {
      entries.forEach((entry) => {
        ...
        if (
          ...
          !typedText.classList.contains("active")
        ) {
   ...
   */
  var navigation_items = document.querySelectorAll(
  ...
  if (accordions) {
    accordions.forEach((item) => {
      item
        .querySelector(".lemmony-accordion-icon")
        .addEventListener("click", (item2) => {
          item2.target.parentNode.classList.toggle("is-open");
        });

      item
        .querySelector(".lemmony-accordion-title")
        .addEventListener("click", (item2) => {
   ...
   */
  var navigation_icons = document.querySelectorAll(
